...
class FaleconoscoController extends ControllerBase
{
	...
	private function index_action($params=null)
	{
		...
		$dados = $this->Delegator('ConcreteFaleconosco', 'BuscarFaleConosco');
			
		//Leva os dados para a view
		$this->View()->assign('dados_faleConosco', $dados);
	...
	private function detalhes()
	{
		...
		if($this->getParam('cod_id') !== false && is_numeric($this->getParam('cod_id')))
		{
			...
			$recordset = $this->Delegator('ConcreteFaleconosco', 'BuscarDetalhesFaleConosco',array("cod_id" => $this->getParam('cod_id')));

			//Verifica o resultado da busca
			if($recordset !== false)
			{
				//Coloca os dados da vaga na View
				$this->View()->assign('dados_faleConosco',$recordset);
			}
			else
			{
	...
	private function exportar()
	{
		...
		if($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			exit();	
			//seleciona tudo
			$resultado = $this->Delegator("ConcreteFaleconosco","SelectFaleconoscofiltrado", $this->getPost());
		}
		...
		elseif($this->getParam())
		{
			...
			//seleciona tudo
			$resultado = $this->Delegator("ConcreteFaleconosco","SelectFaleconoscofiltrado", array('data_de' => $de, 'data_ate' => $ate));
		...
		else
		{
			...
			//seleciona tudo
			$resultado = $this->Delegator("ConcreteFaleconosco","SelectFaleconoscoExportar");
			...
			$this->View()->assign('array_faleConosco',$resultado);
	...
	private function exportar_filtro()
	{
		...
		if($_SERVER['REQUEST_METHOD'] == "POST")
		{
			...
			//seleciona tudo
			$resultado = $this->Delegator("ConcreteFaleconosco","SelectFaleconoscoFiltrar", $this->getPost());
		...
		else
		{
			...
			//seleciona tudo
			$resultado = $this->Delegator("ConcreteFaleconosco","SelectFaleConosco");
		}
	}

...
{
	private $table_alias = "ImprensaNoticias in";
	...
	public function SelectNoticiaRelacaoId($cod_relacao_idioma)
	{
		...
		{
			$query = Doctrine_Query::create()
			->select("in.*, DATE_FORMAT(in.dat_data, '%d/%m/%Y') dat_data, wi.*")
			->from($this->table_alias)
			->innerJoin("in.WebsiteIdiomas wi")
			->where("in.cod_relacao_idioma =?", $cod_relacao_idioma)
			->execute();

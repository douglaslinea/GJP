...
<div class="article-container">
	...
	<ul class="tab-switch">
		<li><a
			onclick="document.location.replace('{view}$URL_DEFAULT{/view}cadastro');"
			href="" rel="tooltip" title="Listar">Listar</a></li>
		<li><a class="default-tab"
			onclick="document.location.replace('{view}$URL_DEFAULT{/view}cadastro/exportar');"
	...
	<div class="tab default-tab" id="tab0">
		<table>
			<tr>
				<form name="frm_filtrar" id="frm_filtrar"
					action="{view}$URL_DEFAULT{/view}cadastro/exportar_filtro"
					method="post">
					...
					<td>
						<button name="btn_filtrar" id="btn_filtrar" type="submit"
							class="gray"
							onclick="document.location.replace('{view}$URL_DEFAULT{/view}cadastro/exportar_filtro');">Filtrar</button>
					</td>
				...
				<td>
					<button name="btn_exportar_periodo" id="btn_exportar_periodo"
						onclick="document.location.replace('{view}$URL_DEFAULT{/view}cadastro/exportar/de/'+(replaceAll(document.getElementById('data_de').value, '/', '_'))+'/ate/'+(replaceAll(document.getElementById('data_ate').value, '/', '_')))"
				...
				<td>
					<button name="btn_mostrar_todos" id="btn_mostrar_todos"
						class="gray"
						onclick="document.location.replace('{view}$URL_DEFAULT{/view}cadastro/exportar')">Voltar
						a mostrar todos</button>
				</td> {view}/if{/view} {view}if $de == '' && $ate == '' {/view}
				<td>
					<button name="btn_exportar_todos" id="btn_exportar_todos"
						onclick="document.location.replace('{view}$URL_DEFAULT{/view}cadastro/exportar_filtro')"
		...
		<table class="datatable">
			...
			<tbody>
				{view}foreach from=$array_cadastro item=cadas{/view}
				<tr>
					<td width="20%">{view}$cadas.dat_cadastro{/view}</td>
					<td width="15%">{view}$cadas.txt_nome{/view}</td>
					<td width="15%">{view}$cadas.txt_email{/view}</td>
					<td width="15%">{view}$cadas.txt_comentario{/view}</td>
				</tr>
				{view}/foreach{/view}
			</tbody>
		</table>
